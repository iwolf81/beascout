#!/usr/bin/env python3
"""
Find Units Without Description Script
Searches scraped HTML files for units that lack a unit-description clause
"""
...
def check_unit_for_description(unit_wrapper):
    """Check if a unit has a description section"""
    # Look for unit-description div within this unit
    description_div = unit_wrapper.find('div', class_='unit-description')
    
    if not description_div:
        return False, "No unit-description div found"
    
    # Check if description has content
    description_text = description_div.get_text(strip=True)
    if not description_text:
        return False, "unit-description div is empty"
    
    # Check if it's just whitespace or minimal content
    if len(description_text) < 10:  # Arbitrary threshold for "meaningful" description
        return False, f"Description too short ({len(description_text)} chars): '{description_text}'"
    
    return True, description_text[:100] + "..." if len(description_text) > 100 else description_text
...
def process_html_file(html_file_path):
    """Process a single HTML file and find units without descriptions"""
    try:
    ...
    units_without_description = []
    total_units = len(unit_wrappers)
    
    for i, wrapper in enumerate(unit_wrappers):
        unit_identifier = extract_unit_identifier(wrapper)
        has_description, description_info = check_unit_for_description(wrapper)
        
        if not has_description:
            units_without_description.append({
                'unit_identifier': unit_identifier,
                'reason': description_info,
                'unit_index': i
            })
    
    return units_without_description, f"Processed {total_units} units"

def main():
    parser = argparse.ArgumentParser(
        description='Find unique units without description clauses in scraped HTML files',
        epilog="""
Examples:
  python scripts/find_units_without_description.py data/scraped/*/beascout_*.html
  python scripts/find_units_without_description.py data/scraped/20250824_220843/beascout_01740.html
  python scripts/find_units_without_description.py "data/scraped/*/beascout_01*.html" --unique-only
        """,
    ...
    print(f"🔍 Searching {len(existing_files)} HTML files for units without descriptions")
    print("=" * 80)
    
    total_units_without_description = 0
    ...
    for html_file in sorted(existing_files):
        ...
        if units_without_desc:
            total_units_without_description += len(units_without_desc)
            ...
            if not args.summary_only and not args.unique_only:
                print(f"\n📄 {Path(html_file).name} - {len(units_without_desc)} units without description:")
                for unit_info in units_without_desc:
                    print(f"  • {unit_info['unit_identifier']}")
                    print(f"    Reason: {unit_info['reason']}")
        elif args.verbose:
            print(f"  ✅ All {file_unit_count} units have descriptions")
    
    # Handle unique-only output
    if args.unique_only and unique_units:
        print(f"\n📋 UNIQUE UNITS WITHOUT DESCRIPTIONS ({len(unique_units)} unique units):")
        print("=" * 80)
    ...
    print(f"  Units without descriptions: {total_units_without_description}")
    print(f"  Unique units without descriptions: {len(unique_units)}")
    print(f"  Files with issues: {len(files_with_issues)}")
    
    if total_units_without_description > 0:
        print(f"  Percentage missing descriptions: {(total_units_without_description/total_units_processed)*100:.1f}%")
