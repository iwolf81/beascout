...
def extract_unit_fields(wrapper, index, unit_name_elem=None):
    ...
    try:
        ...
        for pattern in day_patterns:
            ...
            if day_match:
                ...
                if day_clean.endswith('day'):
                    unit_data['meeting_day'] = day_clean
                    break
        ...
        for pattern in time_patterns:
            ...
            if time_match:
                if time_match.lastindex >= 2 and time_match.group(2) and not time_match.group(2).lower().startswith(('a', 'p')):
                    # Range format: "7:00 - 8:30" or "7 - 8:30"
                    start_time = time_match.group(1)
                    end_time = time_match.group(2)
                    suffix = time_match.group(3) if time_match.lastindex >= 3 and time_match.group(3) else ""
                    ...
                    if suffix:
                        meeting_time += f" {suffix}"
                    unit_data['meeting_time'] = meeting_time
                else:
                    # Single time: "6:30pm"
                    single_time = time_match.group(1)
                    suffix = time_match.group(2) if time_match.lastindex >= 2 else ""
